...
export type ProjectContext = {
  ...
  worktree: string
  /** Identity used to key the local session store. */
  projectId: string
}
 ...
 * a project path. In that case the session directory (`ctx.directory`) is the
 * project, and the id is derived from it so that two different plain folders
 * never share a session store entry or a sandbox.
 */
